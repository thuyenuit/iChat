...
public class GroupDB {
    ...
    public void addGroup(Group group) {
        ...
        for (String idMenber : group.member) {
            ...
            long checkUser = DatabaseUtils.queryNumEntries(db, FeedEntry.TABLE_NAME  +  " WHERE " + FeedEntry.COLUMN_GROUP_ID +"='"+ group.id +"' and "
                                                                                                        + FeedEntry.COLUMN_GROUP_MEMBER +"='"+ idMenber+"'");
    ...
    public ArrayList<Group> getListGroups(String userId) {
        ...
        try {
            Cursor cursor = db.rawQuery("select * from " + GroupDB.FeedEntry.TABLE_NAME
                                            + " where " + FeedEntry.COLUMN_GROUP_MEMBER + "= " +  userId,
                                        null);
            while (cursor.moveToNext()) {
                String idGroup = cursor.getString(0);
                String nameGroup = cursor.getString(1);
